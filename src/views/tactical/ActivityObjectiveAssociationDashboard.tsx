...
const ObjectiveActivityAssociationDashboard: FC<DispatchProp & Props> = ({
  ...
  const associatedObjectiveDictionary = reduceRight(
    allObjectives.filter(
      objective =>
        (objective.associatedActivities || []).indexOf(activityId) > -1,
    ),
  ...
  const selectedObjectives = reduceRight(
    allObjectives,
    (accum: StringDictionary<boolean>, objective) => {
  ...
  const [objectiveSwitches, setActivitySwitches] = useState<
  ...
  const saveActivity = () => {
    const updatedObjectives = objectToKeyValueArray(objectiveSwitches).map(
      idToObjective => {
      ...
      <div className={classes.headerContent}>
        <Container maxWidth={'sm'}>
          ...
          {createAssociationComponent(
            null,
            mappedTacticalActivites[activityId],
          )}
        </Container>
      </div>
                ...
                <div key={objective.id} className={classes.activity}>
                  <Card>
                    <div className={classes.content}>
                      ...
                      <div>
                        <Switch
                          checked={objectiveSwitches[objective.id]}
